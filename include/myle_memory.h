...
namespace MYLE {
	...
	public:
		...
		*/
		template<typename T, typename... TArgs>
		inline T* AllocateResource(uint32_t blockID, TArgs... Args)
		{
			...
			if (index != -1)
			{
				...
				if (block.m_Segments.size() - 1 < index + 1)
				{
					end = block.m_BlockSize;
				}
				else
				{
					end = block.m_Segments.at(index + 1).m_Begin;
				}
		...
		*/
		template<typename T>
		inline void FreeResource(T* resource)
		{
		...
		*/
		inline void AllocateMemoryBlock(uint32_t id, uint32_t size)
		{
		...
		*/
		inline void FreeMemoryBlock(uint32_t index)
		{
